...
from ..exceptions import InvalidStringFormat, SizeMissmatchError, NetworkOriginError
...
class OneDimensionalArrayBase(BaseModel):
    ...
    def array_is_valid(cls, value):
        ...
        assert type(eval(value)) == list, InvalidStringFormat(
            "`array` attribute must be a 'list string'."
        )
        return value
