...
def test_upload_file():
    with open(PRIVATE_NETWORK_FILE, "rb") as f:
        response = client.post(
            "/files/networks", files={"file": (PRIVATE_NETWORK_FILE, f, "image/jpeg")}
        )
        assert response.status_code == 200, response.text
        data = response.json()
...
def test_scan_private_network():
    response = client.get("/networks/", params={"skip": 0, "limit": 1})
    id = response.json()["id"]
...
def test_update_network():
    ...
    response = client.put(f"/networks/{id}", json={"label": UPDATED_LABEL})
    assert response.status_code == 200, response.text
    ...
    response = client.put(
        f"/networks/{id}", json={"origin": UPDATED_PRIVATE_NETWORK_FILE}
    )
    assert response.status_code == 200, response.text
    ...
    assert len(data) == 1
    assert data["updates"] == {
        "origin": f"{PRIVATE_NETWORK_FILE} -> {UPDATED_PRIVATE_NETWORK_FILE}"
    }
