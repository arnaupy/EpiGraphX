...
def compile_cpp_modules(dirpath: str) -> list[CMakeExtension]: 
    ...
    for module_name, module_files in config["modules"].items():
        ...
        for module_file in module_files:
            ...
            source_file = os.path.join(dirpath, config["source_directory"], f"{module_file}.cpp") 
            ...
            ext_modules.append(CMakeExtension(f"{config['package_name']}.{module_name}.{module_file}", sources = [source_file]))
...
def main():    
    ...
    if os.getenv("BUILDING_ENVIRONMENT") in ["production", "test"]:
